...
import { Eye, Check, X, Send, User, Bot, DollarSign, Clock, CheckCircle, XCircle, Sparkles, Crown, TrendingUp, Download, Filter, Search } from 'lucide-react';
...
export default function AdminPayoutsPage() {
  ...
  const [selectedRequest, setSelectedRequest] = useState<PayoutDetails | null>(null);
  ...
  const handleViewDetails = async (request: PayoutRequest) => {
    try {
      const response = await api.get<PayoutDetails>(`/admin/payouts/${request.id}`, {
    ...
    try {
      ...
      toast.promise(promise, {
        ...
        success: () => {
          ...
          fetchRequests();
          return t('PayoutsPage.toast.updateSuccess', { id: selectedRequest.id });
        },
  ...
  const getStatusBadge = (status: string) => {
    ...
    const config = statusMap[status as keyof typeof statusMap] || { icon: <Clock />, label: status, className: "bg-gray-100 text-gray-800" };
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
        ...
        <Card className="bg-white/80 backdrop-blur-sm border-amber-200 shadow-lg rounded-2xl text-center">
          <CardContent className="p-4">
            ...
            <div className="text-amber-700 text-sm">{t('AdminOrdersPage.status.pending')}</div>
          </CardContent>
        </Card>
        <Card className="bg-white/80 backdrop-blur-sm border-green-200 shadow-lg rounded-2xl text-center">
          <CardContent className="p-4">
            ...
            <div className="text-green-700 text-sm">{t('PayoutsPage.actions.approve')}</div>
          </CardContent>
        </Card>
        <Card className="bg-white/80 backdrop-blur-sm border-red-200 shadow-lg rounded-2xl text-center">
          <CardContent className="p-4">
            ...
            <div className="text-red-700 text-sm">{t('PayoutsPage.actions.reject')}</div>
          </CardContent>
        </Card>
        <Card className="bg-white/80 backdrop-blur-sm border-purple-200 shadow-lg rounded-2xl text-center">
          <CardContent className="p-4">
            ...
            <div className="text-purple-700 text-sm">إجمالي المبالغ المعلقة</div>
          </CardContent>
        </Card>
      </div>
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
            <div className="flex flex-col sm:flex-row gap-4 flex-1">
              <div className="relative flex-1">
                ...
                <Input
                  placeholder={t('PayoutsPage.table.empty').replace('لا توجد طلبات سحب معلقة حاليًا.', 'ابحث بالاسم أو البريد الإلكتروني...')}
                  value={searchTerm}
              ...
              <div className="flex gap-3">
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="border-rose-200 text-rose-700 hover:bg-rose-50 rounded-xl">
                      ...
                      {statusFilter === 'all' ? t('AdminOrdersPage.status.all') : 
                       ...
                       statusFilter === 'approved' ? t('PayoutsPage.actions.approve') : 
                  ...
                  <DropdownMenuContent className="border-rose-200 rounded-xl">
                    ...
                    <DropdownMenuItem onClick={() => setStatusFilter('approved')} className="text-green-600">
                      {t('PayoutsPage.actions.approve')}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setStatusFilter('rejected')} className="text-red-600">
                      {t('PayoutsPage.actions.reject')}
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <Button variant="outline" className="border-rose-200 text-rose-700 hover:bg-rose-50 rounded-xl">
                      <Filter className="w-4 h-4 ml-2" />
                      {typeFilter === 'all' ? 'جميع الأنواع' : 
                       typeFilter === 'merchant' ? t('PayoutsPage.table.merchant') : 'الموردين فقط'}
                    </Button>
                  </DropdownMenuTrigger>
                  <DropdownMenuContent className="border-rose-200 rounded-xl">
                    <DropdownMenuItem onClick={() => setTypeFilter('all')} className="text-rose-700">
                      جميع الأنواع
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setTypeFilter('merchant')} className="text-blue-600">
                      {t('PayoutsPage.table.merchant')}
                    </DropdownMenuItem>
                    <DropdownMenuItem onClick={() => setTypeFilter('supplier')} className="text-purple-600">
                      الموردين فقط
                    </DropdownMenuItem>
                  </DropdownMenuContent>
                </DropdownMenu>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-pink-100">
                {t('PayoutsPage.table.empty').replace('لا توجد طلبات سحب معلقة حاليًا.', `مراجعة وإدارة جميع طلبات السحب (${filteredRequests.length} طلب)`)}
          ...
          <Table>
            <TableHeader>
              <TableRow className="bg-rose-50/50 hover:bg-rose-50/70">
                <TableHead className="text-rose-800 font-bold">{t('PayoutsPage.dialog.name')}</TableHead>
                <TableHead className="text-rose-800 font-bold">النوع</TableHead>
                <TableHead className="text-rose-800 font-bold">{t('PayoutsPage.table.amount')}</TableHead>
                <TableHead className="text-rose-800 font-bold">{t('AdminOrdersPage.table.status')}</TableHead>
                <TableHead className="text-rose-800 font-bold">{t('PayoutsPage.table.date')}</TableHead>
            ...
            <TableBody>
              {loading ? (
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-12">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <p className="text-rose-700 font-medium">{t('PayoutsPage.toast.fetchError')}</p>
                    </div>
                  </TableCell>
                </TableRow>
                ...
                <TableRow>
                  <TableCell colSpan={6} className="text-center py-12">
                    <div className="flex flex-col items-center justify-center">
                      ...
                      <h3 className="font-bold text-xl text-rose-800 mb-2">{t('PayoutsPage.table.empty')}</h3>
                ...
                filteredRequests.map((req) => (
                  <TableRow key={`${req.id}-${req.user_type}`} className="border-rose-100 hover:bg-rose-50/30 transition-colors">
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <div className="font-medium text-rose-900">{req.name}</div>
                          <div className="text-rose-600 text-sm">{req.email}</div>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                      {getUserTypeBadge(req.user_type)}
                    </TableCell>
                    <TableCell>
              ...
              <div className="py-4 space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <Card className="bg-rose-50/50 border-rose-200 rounded-2xl">
                    <CardContent className="p-4">
                      <h4 className="font-semibold text-rose-800 mb-3">{t('PayoutsPage.dialog.merchantInfo')}</h4>
                      <div className="space-y-2 text-sm">
                        <p><strong>{t('PayoutsPage.dialog.name')}:</strong> {selectedRequest.name}</p>
                        <p><strong>{t('PayoutsPage.dialog.email')}:</strong> {selectedRequest.email}</p>
                        {selectedRequest.phone_number && (
                          <p><strong>{t('PayoutsPage.dialog.phone')}:</strong> {selectedRequest.phone_number}</p>
                        )}
                      </div>
                    </CardContent>
                  </Card>
                  ...
                  <Card className="bg-blue-50/50 border-blue-200 rounded-2xl">
                    <CardContent className="p-4">
                      <h4 className="font-semibold text-blue-800 mb-3">{t('PayoutsPage.dialog.bankInfo')}</h4>
                      <div className="space-y-2 text-sm">
                        <p><strong>{t('PayoutsPage.dialog.accountNumber')}:</strong> {selectedRequest.account_number || t('PayoutsPage.dialog.notAvailable')}</p>
                        <p><strong>{t('PayoutsPage.dialog.iban')}:</strong> {selectedRequest.iban || t('PayoutsPage.dialog.notAvailable')}</p>
