...
const ReelItem = memo(({ 
  ...
  // Register video element when component mounts
  useEffect(() => {
    if (videoRef.current) {
...
export const ReelsSlider: React.FC<ReelsSliderProps> = ({ reels: initialReels, autoPlay = true }) => {
  ...
  useEffect(() => {
    ...
    const handleIntersection = (entries: IntersectionObserverEntry[]) => {
      ...
      // Play the most visible reel, pause others
      if (mostVisibleReel && mostVisibleReel.visibility >= 0.7) {
        ...
        // If no video is sufficiently visible, pause the currently playing one
  ...
  // Observe video elements when they're mounted
  useEffect(() => {
  ...
  // Rest of the functions (handleLikeToggle, openComments, handleShare, handleReport) remain the same
