        ...
        {stories.length === 0 ? (
          ...
          <div className="p-3 sm:p-4 space-y-3">
            {stories.map((story) => (
              <div
                key={story.id}
                className="border border-blue-200 rounded-xl p-3 bg-blue-50/30"
              >
                <div className="flex gap-3">
                  <div className="w-12 h-12 rounded-lg bg-gradient-to-br from-blue-50 to-indigo-50 overflow-hidden relative border border-blue-200">
                    {story.type === 'text' ? (
                      <div className="w-full h-full flex items-center justify-center text-[6px] text-center p-1 bg-gradient-to-br from-blue-500 to-indigo-500 text-white leading-tight">
                  ...
                  <div className={`flex-1 ${isRTL ? 'text-right' : 'text-left'} space-y-1`}>
                    ...
                    <div className="flex items-center gap-1 text-blue-900 text-sm">
                  ...
                  <div>
                    <AlertDialog>
                      ...
                      <AlertDialogContent className="bg-white/95 backdrop-blur-sm border-blue-200 rounded-2xl p-4">
                        <AlertDialogHeader>
                          <AlertDialogTitle className="text-blue-800 text-lg">
                            {t('StoriesManager.delete.confirmTitle')}
                          </AlertDialogTitle>
                          <AlertDialogDescription className="text-blue-600 text-sm">
                            {t('StoriesManager.delete.confirmMessage')}
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter className="gap-2 pt-2">
                          <AlertDialogCancel className="rounded-lg bg-white border-blue-200 text-blue-700 hover:bg-blue-50 text-sm">
                            {t('common.cancel')}
                          </AlertDialogCancel>
                          <AlertDialogAction
            ...
            <Table>
              <TableHeader>
                <TableRow className="border-blue-100 hover:bg-transparent">
                  <TableHead className={`text-blue-800 font-bold whitespace-nowrap ${isRTL ? 'text-right' : 'text-left'}`}>
                    <ImageIcon className="inline-block ml-1 h-4 w-4" />
                    {t('StoriesManager.table.preview')}
                  </TableHead>
                  <TableHead className={`text-blue-800 font-bold ${isRTL ? 'text-right' : 'text-left'}`}>
                ...
                {stories.map((story) => (
                  ...
                  >
                    <TableCell className="p-3 sm:p-4">
                      <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 overflow-hidden relative border border-blue-200 shadow-sm">
                        {story.type === 'text' ? (
                          <div className="w-full h-full flex items-center justify-center text-[6px] text-center p-1 bg-gradient-to-br from-blue-500 to-indigo-500 text-white leading-tight">
                            {story.text_content?.substring(0, 20) || ''}
                          </div>
                    ...
                    <TableCell className={isRTL ? 'text-right' : 'text-left'}>
                      <div className="flex items-center justify-start gap-1 font-semibold text-blue-900">
                        <Eye size={16} className="text-blue-600" />
                        {story.views.toLocaleString()}
                      </div>
                    </TableCell>
                    <TableCell className={`text-blue-700 text-sm ${isRTL ? 'text-right' : 'text-left'}`}>
                      {format(new Date(story.created_at), 'h:mm a', { locale })}
                    </TableCell>
                    <TableCell className={isRTL ? 'text-right' : 'text-left'}>
                    ...
                    <TableCell className={isRTL ? 'text-right' : 'text-left'}>
                      <AlertDialog>
                        ...
                        <AlertDialogContent className="bg-white/95 backdrop-blur-sm border-blue-200 rounded-3xl">
                          <AlertDialogHeader>
                          ...
                          <AlertDialogFooter>
                            <AlertDialogCancel className="rounded-xl bg-white border-blue-200 text-blue-700 hover:bg-blue-50">
                              {t('common.cancel')}
                            </AlertDialogCancel>
                            <AlertDialogAction
                              onClick={() => handleDeleteStory(story.id)}
