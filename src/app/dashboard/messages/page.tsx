...
import { Send, MessageSquare, Paperclip, Check, CheckCheck, FileText, Menu, X } from 'lucide-react';
...
function MessagesPage() {
  ...
  const handleSendMessage = async (body: string | null, attachment_url: string | null = null, attachment_type: string | null = null) => {
  ...
  return (
    <div className="min-h-screen flex flex-col p-2 sm:p-4 md:p-6">
      ...
      <div className="flex-1 flex flex-col">
        <h1 className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6">{t('MessagesPage.title')}</h1>
        ...
        <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-4 gap-3 sm:gap-4">
          {/* Conversations List */}
          <Card className={`lg:col-span-1 flex flex-col transition-all duration-300 ${
            ...
            <div className="flex-1 overflow-y-auto min-h-0">
              {loadingConversations ? (
                <p className="p-4 text-sm text-gray-500">{t('MessagesPage.loadingConversations')}</p>
                ...
                conversations.map(convo => (
                  <div
                    ...
                    className={`flex items-center p-3 m-2 rounded-lg cursor-pointer transition-colors ${
                      activeConversation?.id === convo.id ? 'bg-purple-100' : 'hover:bg-gray-100'
                    }`}
                  >
                ...
                <p className="p-4 text-center text-sm text-gray-500">{t('MessagesPage.noConversations')}</p>
              )}
            </div>
          </Card>
          ...
          <Card className={`lg:col-span-3 flex flex-col ${
            ...
            {activeConversation ? (
              <>
                <div className="p-3 sm:p-4 border-b flex items-center justify-between flex-shrink-0">
                  <div className="flex items-center">
                    ...
                    <div className="mr-3">
                      <p className="font-bold text-sm sm:text-base">{activeConversation.participantName}</p>
                            ...
                            {msg.attachment_url && (
                              msg.attachment_type === 'image' ? (
                                ...
                                >
                                  ...
                                  <span className="text-sm">{t('MessagesPage.attachmentFile')}</span>
                                </Link>
                              )
                            )}
                          </div>
              ...
              <div className="flex flex-col items-center justify-center h-full text-gray-500 p-4">
                ...
                <p className="font-semibold text-sm sm:text-base">{t('MessagesPage.selectConversation')}</p>
                <p className="text-xs sm:text-sm">{t('MessagesPage.selectConversationHint')}</p>
              </div>
            )}
          </Card>
        </div>
