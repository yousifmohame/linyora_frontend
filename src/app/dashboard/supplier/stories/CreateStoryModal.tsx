  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogTrigger asChild>
        <Button className="gap-1 sm:gap-2 rounded-xl bg-gradient-to-r from-red-500 to-red-500 hover:from-red-600 hover:to-red-600 text-white font-bold text-xs sm:text-sm px-3 py-2 sm:px-4 sm:py-2.5 transition-all duration-200 shadow-md sm:shadow-lg">
      ...
      <DialogContent className="max-w-[95vw] sm:max-w-[500px] bg-white/95 backdrop-blur-sm border-blue-200 rounded-2xl sm:rounded-3xl shadow-xl overflow-hidden">
        <DialogHeader className="bg-gradient-to-r from-blue-500 to-indigo-500 text-white rounded-t-xl sm:rounded-t-2xl p-4 sm:p-6 -m-4 sm:-m-6 mb-4 sm:mb-6">
        ...
        <Tabs
          ...
          className="w-full"
        >
          <TabsList className="grid w-full grid-cols-3 bg-gradient-to-r from-blue-50 to-indigo-50 p-1 rounded-xl sm:rounded-2xl mb-4">
            <TabsTrigger
              value="image"
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            <Card className="bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200 rounded-xl sm:rounded-2xl overflow-hidden">
              <CardContent className="p-3 sm:p-6">
                {(activeTab === 'image' || activeTab === 'video') && (
                  <div
                    className="border-2 border-dashed border-blue-300 rounded-xl sm:rounded-2xl p-4 sm:p-6 text-center cursor-pointer hover:bg-blue-50/50 transition-colors bg-white/50"
                  ...
                  >
                    ...
                    {filePreview ? (
                      ...
                      <p className="text-xs sm:text-sm font-medium text-blue-600 bg-blue-50 py-1 px-2 sm:py-2 sm:px-3 rounded-lg">
                      ...
                      <div className="flex flex-col items-center text-blue-500">
                        ...
                        <p className="font-semibold text-blue-700 text-xs sm:text-sm">
                ...
                <div className="space-y-2 sm:space-y-3 mt-4">
                  <label className="text-xs sm:text-sm font-medium text-blue-800 flex items-center gap-1.5">
                  ...
                  <Textarea
                    ...
                    className="border-blue-200 focus:border-blue-400 rounded-lg sm:rounded-xl min-h-[60px] sm:min-h-[80px] text-xs sm:text-sm bg-white/50"
                  />
                </div>
                        ...
                        <button
                          ...
                          className={`relative w-full aspect-square rounded-lg sm:rounded-xl border transition-transform duration-200 hover:scale-105 ${
                            watch('background_color') === color.value
                              ? 'border-white ring-2 ring-blue-300 shadow'
                              : 'border-blue-200'
                          }`}
                ...
                {user?.role_id === 1 && (
                  <div className="space-y-3 mt-4 pt-4 border-t border-blue-200">
                    <div className="flex items-center gap-1.5">
                      ...
                      <label className="text-xs sm:text-sm font-semibold text-blue-800 uppercase tracking-wide">
                        {t('CreateStoryModal.admin.title')}
                      </label>
                    </div>

                    <Select onValueChange={(val) => setValue('section_id', val)}>
                      <SelectTrigger className="bg-white border-blue-200 focus:border-blue-400 rounded-lg sm:rounded-xl text-xs sm:text-sm">
                        <SelectValue placeholder={t('CreateStoryModal.admin.sectionPlaceholder')} />
                      </SelectTrigger>
                      <SelectContent className="bg-white border-blue-200 rounded-lg sm:rounded-xl max-h-48">
                        <SelectItem
                          value="personal"
                          className="text-blue-800 hover:bg-blue-50 text-xs sm:text-sm"
                        >
                        ...
                        {loadingSections ? (
                          <div className="flex items-center justify-center p-3">
                            ...
                            <span className="mr-1.5 text-xs sm:text-sm text-blue-600">
                              {t('common.loading')}
                            </span>
                          </div>
                        ) : sections.length > 0 ? (
                          ...
                          <div className="p-2 text-center text-[10px] sm:text-xs text-blue-400">
                            {t('CreateStoryModal.admin.noSections')}
                          </div>
                        )}
                      </SelectContent>
                    </Select>
                    <p className="text-[10px] sm:text-xs text-blue-500 bg-blue-50 p-1.5 sm:p-2 rounded-lg">
            ...
            <Button
              type="submit"
              className="w-full rounded-xl bg-gradient-to-r from-blue-500 to-indigo-500 hover:from-blue-600 hover:to-indigo-600 text-white font-bold text-sm sm:text-base py-2.5 sm:py-3 transition-all duration-200 shadow-md sm:shadow-lg"
