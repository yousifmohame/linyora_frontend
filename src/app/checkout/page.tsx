...
function CheckoutForm() {
  ...
  const fetchAddresses = async () => {
    try {
      ...
      if (!selectedAddressId && addrRes.data.length > 0) {
        const defaultAddr = addrRes.data.find(a => a.is_default);
        setSelectedAddressId(defaultAddr?.id || addrRes.data[0].id);
      }
    } catch (error) {
  ...
  const fetchCards = async () => {
    try {
      ...
      console.warn('Failed to fetch cards');
    }
  };
        ...
        const updatedGroups = await Promise.all(
          groupsArray.map(async (group) => {
            try {
              ...
              const res = await api.post<ShippingOption[]>('/products/shipping-options-for-cart', {
                productIds,
              });
              return {
                ...group,
  ...
  const handleShippingChange = (merchantId: string | number, shippingId: string) => {
    setMerchantGroups((prev) =>
      prev.map((g) => (g.merchantId === merchantId ? { ...g, selectedShippingId: parseInt(shippingId) } : g))
    );
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50/50 p-4 md:p-8" dir={isRTL ? 'rtl' : 'ltr'}>
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            {/* Address */}
            <Card className="border-0 shadow-sm ring-1 ring-gray-200 rounded-2xl">
          ...
          <div className="h-2 w-2 bg-white rounded-full" />
                  ...
                  <Dialog open={isAddressFormOpen} onOpenChange={setIsAddressFormOpen}>
                    <DialogTrigger asChild>
                      <Button
                        variant="outline"
                        className="w-full border-dashed border-gray-300 h-12 text-gray-500 hover:text-purple-600 hover:border-purple-200 hover:bg-purple-50"
                      >
            ...
            {merchantGroups.map((group) => (
              <Card key={group.merchantId} className="border-0 shadow-sm ring-1 ring-gray-200 rounded-2xl">
                <CardHeader className="bg-white border-b border-gray-100 pb-4">
                        ...
                        {group.shippingOptions.map((opt) => (
                          <div
                            ...
                            className={`flex items-center justify-between p-3 rounded-lg border-2 cursor-pointer transition-all ${
                              group.selectedShippingId === opt.id
                ...
                <RadioGroup value={paymentMethod} onValueChange={(v) => setPaymentMethod(v)} className="space-y-4">
                  <div
                    className={`border-2 rounded-xl p-4 transition-all ${
                      paymentMethod === 'card' ? 'border-blue-500 bg-blue-50/20' : 'border-gray-200'
                    }`}
                  >
                              ...
                              <div className="flex items-center gap-3">
                                <div
                                  className={`w-4 h-4 rounded-full border flex items-center justify-center ${
                                    selectedCardId === card.id ? 'border-blue-600' : 'border-gray-400'
                                  }`}
                                >
                  ...
                  <div
                    className={`flex items-center gap-3 p-4 border-2 rounded-xl cursor-pointer transition-all ${
                      paymentMethod === 'cod' ? 'border-green-500 bg-green-50/20' : 'border-gray-200'
                    }`}
                  >
                ...
                <CardContent className="p-6 space-y-4">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-purple-600">
                      {finalTotal.toFixed(2)} <span className="text-sm font-normal text-gray-500">{t('common.currency')}</span>
                    </span>
                  </div>
                  <div className="bg-blue-50 p-3 rounded-lg flex items-start gap-2 text-xs text-blue-700 mt-4">
                    ...
                    <p>
                      {t('checkout.secureCheckoutNote', {
                        defaultValue: 'بياناتك محمية بتشفير 256-bit SSL. نحن نضمن تجربة دفع آمنة 100%.',
                      })}
                    </p>
                  </div>
                  <Button
                    className="w-full h-12 text-base font-bold bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 rounded-xl mt-4"
                  ...
                  >
                    {isProcessing ? (
                      ...
                      <>
                        ...
                        {t('checkout.payNow')} {finalTotal.toFixed(2)} {t('common.currency')}
                      </>
                    )}
                  </Button>
                </CardContent>
